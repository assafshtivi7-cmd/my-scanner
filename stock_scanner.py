...
MARKET_INDICES = {
    "S&P 500": "^GSPC", "NASDAQ": "^IXIC", "BITCOIN": "BTC-USD", "VIX (FEAR)": "^VIX"
}
...
def generate_html(results, market_summary):
    ...
    def score_cls(s): return {4:"s4", 3:"s3", 2:"s2"}.get(s, "s1")
    def trend_cls(t):
...
:root {{
  ...
  --up: #3fb950; --dn: #f85149; --mono: 'IBM Plex Mono', monospace;
}}
body {{ background: var(--bg); color: var(--text); font-family: var(--mono); font-size: 12px; margin: 0; }}
.topbar {{ background: var(--surface); border-bottom: 1px solid var(--border); padding: 0 28px; height: 48px; display: flex; align-items: center; justify-content: space-between; sticky: top; z-index: 100; }}
.brand {{ font-weight: 700; color: #e2e8f0; display: flex; align-items: center; gap: 12px; }}
.live-dot {{ width: 6px; height: 6px; border-radius: 50%; background: var(--up); box-shadow: 0 0 6px var(--up); animation: blink 2.4s infinite; }}
...
.idx-bar {{ position: absolute; top: 0; left: 0; bottom: 0; width: 2px; }}
.up-bar {{ background: var(--up); }} .dn-bar {{ background: var(--dn); }}
.idx-val {{ font-size: 20px; font-weight: 700; color: #e2e8f0; }}
.ticker {{ background: var(--gold) !important; color: #000 !important; font-weight: 800; padding: 3px 9px; border-radius: 4px; min-width: 60px; display: inline-block; text-align: center; }}
...
.s4 {{ background: rgba(63,185,80,0.1); color: var(--up); border: 1px solid var(--up); }}
.s3 {{ background: rgba(201,170,113,0.1); color: var(--gold); border: 1px solid var(--gold); }}
/* Modal */
.modal-content {{ background: var(--surface); border: 1px solid var(--border); color: #fff; }}
.modal-body {{ height: 600px; padding: 0; }}
</style>
</head>
<body>
<div class="topbar"><div class="brand"><div class="live-dot"></div>SHTIVI <span style="color:var(--gold)">COMMAND CENTER</span></div><div style="font-size:10px">IDT {israel_time}</div></div>
...
def create_styled_excel(df, file_name):
    ...
    ws.conditional_format(1, 2, len(df), 2, {'type':'cell', 'criteria':'>=', 'value':4, 'format':wb.add_format({'bg_color':'#C6EFCE'})})
    writer.close()
...
if __name__ == "__main__":
    ...
    prev = json.load(open(DB_FILE)) if os.path.exists(DB_FILE) else {}
    results = []
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        futures = {ex.submit(analyze_ticker, t, spy_ret, prev): t for t in WATCHLIST}
    ...
    f_name = f"Master_Scanner_{datetime.now(timezone(timedelta(hours=3))).strftime('%Y-%m-%d')}.xlsx"
    ...
    # שליחת מייל (כאן תדביק את ה-send_email המלא שלך)
