...
def get_data():
    results, market_summary, all_stocks = {}, [], []
    ...
    for cat, tickers in WATCHLIST.items():
        ...
        for t in tickers:
            try:
                ...
                curr, chg = s['Close'].iloc[-1], ((s['Close'].iloc[-1] - s['Close'].iloc[-2]) / s['Close'].iloc[-2]) * 100
                stock_obj = {"ticker": t, "price": f"{curr:.2f}", "change": f"{chg:+.2f}%", "raw_chg": chg}
                results[cat].append(stock_obj)
                all_stocks.append(stock_obj)
            except: continue
    
    # חילוץ 3 המניות המובילות
    top_performers = sorted(all_stocks, key=lambda x: x['raw_chg'], reverse=True)[:3]
    return results, market_summary, top_performers

def generate_html(data, market, top):
    now = datetime.now().strftime("%d/%m/%Y %H:%M")
    
    # עיצוב מניות מובילות
    top_html = "".join([f'<div class="col-md-4"><div class="card bg-success text-white mb-3"><div class="card-body">🚀 {s["ticker"]}: {s["change"]}</div></div></div>' for s in top])
    
    market_cards = "".join([f'<div class="col-md-4 mb-3"><div class="card text-center border-{m["color"]}"><div class="card-body"><h5>{m["name"]}</h5><h3>{m["price"]}</h3><span class="badge bg-{m["color"]}">{m["change"]}</span></div></div></div>' for m in market])
...
def send_email(top):
    ...
    try:
        ...
        msg = MIMEMultipart()
        msg['Subject'] = f"Daily Top Stocks {datetime.now().strftime('%d/%m/%Y')}"
